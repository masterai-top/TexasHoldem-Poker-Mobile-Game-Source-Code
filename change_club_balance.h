...
class ChangeClubBalanceRequest
{
    friend class ChangeClubBalanceRespons;
public:
    ChangeClubBalanceRequest() {}
    ChangeClubBalanceRequest(const std::string &json)
    {
...
class ChangeClubBalanceRespons
{
public:
    ChangeClubBalanceRespons() {}
    ChangeClubBalanceRespons(const string &json)
    {
        this->Deserialize(json);
    }
    template <typename Writer>
    ...
    static tars::Int32 handler(const vector<tars::Char> &reqBuf, const map<std::string, std::string> &extraInfo, vector<tars::Char> &rspBuf)
    {
        ...
        // STEP1 解码
        ChangeClubBalanceRequest request;
        ...
        Club::InnerClubInfoCoinResp resp;
        req.cId = S2L(request._sClubID);
        req.uId = S2L(request._sUid);
        req.flag = 1;
        req.amount = S2L(request._sAmount);
        int iRet = g_app.getOuterFactoryPtr()->getSocialServerPrx(req.cId)->InnerClubInfoCoin(req, resp);
        if (iRet != 0)
        {
            ROLLLOG_ERROR << "InnerClubInfoCoin failed, iRet:" << iRet << ", request._sClubID:" << req.cId << endl;
...
private:
    ...
    static void encode(int64_t resultCode, ChangeClubBalanceRequest &request, vector<tars::Char> &rspBuf)
    {
        ChangeClubBalanceRespons  response;
