...
class ChangeClubFundRequest
{
    friend class ChangeClubFundRespons;
public:
    ChangeClubFundRequest() {}
    ChangeClubFundRequest(const std::string &json)
    {
    ...
    void Serialize(Writer &writer) const
    {
        ...
        SERIALIZE_MEMBER(writer, sAmount);
        writer.EndObject();
    }
    ...
    void Deserialize(const string &json)
    {
        try
        {
            ...
            SET_DOC_MEMBER(d, sAmount);
        }
        catch (const std::exception &e)
        {
            std::string errInfo = ::toString(__FILE__, ":", __LINE__, ":ChangeClubFundRequest decode error!");
...
private:
    ...
    CString _sLevel;        // 商户等级
    CString _sAmount;       // 增加筹码
};
class ChangeClubFundRespons
{
public:
    ChangeClubFundRespons() {}
    ChangeClubFundRespons(const string &json)
    {
        this->Deserialize(json);
    }
    template <typename Writer>
    ...
    static tars::Int32 handler(const vector<tars::Char> &reqBuf, const map<std::string, std::string> &extraInfo, vector<tars::Char> &rspBuf)
    {
        ...
        // STEP1 解码
        ChangeClubFundRequest request;
        decode(reqBuf, request);

        // STEP2 具体业务处理
        int64_t resultCode = RESULT_CODE_SUCCESS;
       
        Club::InnerClubInfoFundReq req;
        Club::InnerClubInfoFundResp resp;
        req.cId = S2L(request._sClubID);
        req.uId = S2L(request._sUid);
        req.flag = 6;
        req.level = S2I(request._sLevel);
        req.amount = S2L(request._sAmount);
        int iRet = g_app.getOuterFactoryPtr()->getSocialServerPrx(req.cId)->InnerClubInfoFund(req, resp);
        if (iRet != 0)
        {
            ROLLLOG_ERROR << "InnerClubInfoFund failed, iRet:" << iRet << ", request._sClubID:" << req.cId << endl;
...
private:
    ...
    static void encode(int64_t resultCode, ChangeClubFundRequest &request, vector<tars::Char> &rspBuf)
    {
        ChangeClubFundRespons  response;
